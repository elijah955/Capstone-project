...
if (!isset($_SESSION['emp_id'])){
	...
	echo ("<center>"); 
	echo ("<p><h3>Customer Payment Information</h3></p>");
	echo ("<p><a href=searchformpay.php class='search'>Search Customer</a></p>"); 
	...
	<th>First Name</th><th>Last Name</th><th>Phone #</th><th>Payment Method</th><th>Total Balance Due</th><th>*</th></tr>"; 
	...
	if($pages>1){
		...
		if($current_page != 1){
			echo '<td><a href="checkout2.php?s='. ($start - $display) . '&p=' . $pages. '"> Previous </a></td>';
		}
		//Make all the numbered pages:
		for($i = 1; $i <= $pages; $i++){
			if($i != $current_page){ // if not the current pages, generates links to that page
				echo '<td><a href="checkout2.php?s='. (($display*($i-1))). '&p=' . $pages .'"> ' . $i . ' </a></td>';
		...
		if($current_page != $pages){
			echo '<td><a href="checkout2.php?s=' .($start + $display). '&p='. $pages. '"> Next </a></td>';
		}
