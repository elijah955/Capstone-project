...
if (!isset($_SESSION['emp_id'])){
	...
	echo ("<center>"); 
	echo ("<h3>Customers With Outstanding Balance</h3><p>");
	echo '<button onClick="window.print()" class="print">Print this page</button>';
	...
	if (isset($_GET['p']) && is_numeric($_GET['p'])){//Already been determined
		...
		$q = "SELECT COUNT(ID) FROM customers WHERE balance!=0";
	...
	$query = "SELECT * FROM customers WHERE balance!=0 LIMIT $start, $display"; 
	$result = @mysqli_query ($dbc, $query);

	//Table header:
	echo "<table cellpadding=5 cellspacing=5 border=1><tr>
	<th>First Name</th><th>Last Name</th><th>Phone #</th><th>Address</th><th>balance</th></tr>"; 
	...
	if($pages>1){
		...
		if($current_page != 1){
			echo '<td><a href="reportrun.php?s='. ($start - $display) . '&p=' . $pages. '"> Previous </a></td>';
		}
		//Make all the numbered pages:
		for($i = 1; $i <= $pages; $i++){
			if($i != $current_page){ // if not the current pages, generates links to that page
				echo '<td><a href="reportrun.php?s='. (($display*($i-1))). '&p=' . $pages .'"> ' . $i . ' </a></td>';
		...
		if($current_page != $pages){
			echo '<td><a href="reportrun.php?s=' .($start + $display). '&p='. $pages. '"> Next </a></td>';
		}
